...
for(let i = 0; i < galleryImg.length; i++) {
  galleryImg[i].addEventListener('click', function(e) {
    ...
    if(eleccion == opPc) 
      document.getElementById('resultado').innerHTML = `<b>Empate</b> los dos sacaron ${eleccion}`;
     else if ((eleccion == "tijera" && opPc == "papel") || (eleccion == "papel" && opPc == "piedra") || (eleccion == "piedra" && opPc == "tijera")) 
       document.getElementById('resultado').innerHTML = `<b>Ganaste</b> sacando ${eleccion} contra ${opPc}`;
      else 
      document.getElementById('resultado').innerHTML = `<b>Perdiste</b> sacando ${eleccion} contra ${opPc}`;
  }
  )
